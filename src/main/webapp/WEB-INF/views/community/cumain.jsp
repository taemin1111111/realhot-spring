    ...
    <div class="category-grid">
        <% for(CCategoryDto cat : categories) { %>
            <div class="category-card" data-category-id="<%= cat.getId() %>">
                ...
                <button class="category-btn" onclick="window.loadCategoryPosts(<%= cat.getId() %>, '<%= cat.getName() %>')">
                    구경가기
                </button>
            </div>
        <% } %>
...
function checkLoginStatus() {
    ...
    if (token) {
        try {
            ...
            isLoggedIn = payload.exp > currentTime;
        } catch (error) {
...
// 핵심 전역 함수들을 먼저 정의
window.loadCategoryPosts = function(categoryId, categoryName, forceReload = false) {
    // 글 상세보기에서 목록으로 돌아가는 경우는 강제 로드
    const isInDetailView = document.querySelector('.post-detail-container') !== null;
    
    if (window.currentCategoryId == categoryId && !forceReload && !isInDetailView) {
        return;
    }
    ...
    fetch('<%=root%>/api/community/posts?category=' + categoryId)
...
window.likePost = function(postId) {
    fetch('<%=root%>/api/community/post/action?action=like&id=' + postId)
...
window.dislikePost = function(postId) {
    fetch('<%=root%>/api/community/post/action?action=dislike&id=' + postId)
        .then(response => response.json())
        .then(data => {
            if(data.success) {
                document.getElementById('likes-count').textContent = data.likes;
...
window.loadPostDetail = function(postId) {
    ...
    // 조회수 증가
    fetch('<%=root%>/api/community/post/action?action=view&id=' + postId)
        .then(response => response.json())
        .then(data => {
            // 조회수 증가 후 글 로드
            return fetch('<%=root%>/api/community/post/detail?id=' + postId);
        })
        .then(response => response.text())
        .then(html => {
            // 부드러운 교체를 위한 딜레이
...
function checkReportStatus(postId) {
    fetch('<%=root%>/api/community/post/action?action=checkReport&id=' + postId)
        .then(response => response.json())
        .then(data => {
...
function submitReport() {
    ...
    fetch('<%=root%>/api/community/post/action', {
        method: 'POST',
        headers: {
...
function loadComments(postId) {
    fetch('<%=root%>/api/community/comments?action=list&post_id=' + postId)
        .then(res => res.json())
        .then(data => {
...
// 현재 정렬 상태를 유지하면서 댓글 새로고침
function reloadCommentsWithCurrentSort(postId) {
    // 현재 활성화된 정렬 버튼 찾기
    const activeBtn = document.querySelector('.comment-sort-buttons .sort-btn.active');
    // 전역 변수 사용
    
    if (activeBtn) {
        // 버튼 텍스트로 정렬 타입 판단
        if (activeBtn.textContent.includes('인기순')) {
    ...
    // 현재 정렬 상태로 댓글 로드
    loadCommentsWithSort(postId, window.currentSort);
}

function likeComment(commentId) {
    fetch('<%=root%>/api/community/comment/vote?action=like&comment_id=' + commentId)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function dislikeComment(commentId) {
    fetch('<%=root%>/api/community/comment/vote?action=dislike&comment_id=' + commentId)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // 해당 댓글의 좋아요/싫어요 수 업데이트
...
function loadCommentsWithSort(postId, sortType) {
    fetch('<%=root%>/api/community/comments?action=list&post_id=' + postId + '&sort=' + sortType)
        .then(res => res.json())
        .then(data => {
            const space = document.getElementById('commentSpace');
            if (space) {
...
function deleteComment(commentId) {
    ...
    fetch('<%=root%>/api/community/comment/action', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            ...
            // 댓글 목록 새로고침 - 현재 정렬 상태 유지
            const postId = document.querySelector('.post-detail-container').dataset.postId;
            reloadCommentsWithCurrentSort(postId);
        } else {
...
// 글쓰기 폼 로드 함수 (전역 함수)
window.loadWriteForm = function() {
    // AJAX로 글쓰기 폼 로드 (현재 카테고리 정보 포함)
    ...
    fetch('<%=root%>/api/community/post/write?category=' + categoryId)
        .then(response => response.text())
        .then(html => {
            // 부드러운 교체를 위한 딜레이
            setTimeout(() => {
        ...
        .catch(() => {
            setTimeout(() => {
                container.innerHTML = '<div class="error-message"><p>글쓰기 폼을 불러오는데 실패했습니다.</p></div>';
                container.classList.remove('loading-transition');
            }, 200);
        });
}
...
window.goBackToList = function() {
    try {
        ...
        let categoryId = window.currentCategoryId || 1; // 기본값 1
        let categoryName = '헌팅썰';
        
        // 글 상세 페이지에서 카테고리 정보를 직접 가져올 수 있는지 확인
        if (postDetailContainer && postDetailContainer.dataset.postId) {
            // URL에서 현재 글의 카테고리 정보를 추론하거나 기본값 사용
            const urlParams = new URLSearchParams(window.location.search);
        ...
        // 카테고리 이름 설정
        switch(categoryId) {
            case 1: categoryName = '헌팅썰'; break;
            case 2: categoryName = '코스 추천'; break;
            case 3: categoryName = '같이 갈래'; break;
            default: categoryName = '커뮤니티'; break;
        }
        
        // 카테고리 목록으로 돌아가기 (강제 로드)
        if (typeof window.loadCategoryPosts === 'function') {
            window.loadCategoryPosts(categoryId, categoryName, true); // forceReload = true
        } else {
            alert('목록 로드 함수를 찾을 수 없습니다. 페이지를 새로고침하세요.');
        }
    } catch (error) {
        alert('목록으로 돌아가기 중 오류가 발생했습니다: ' + error.message);
    }
}
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (firstCard) {
        ...
        window.loadCategoryPosts(categoryId, categoryName);
        }
    }
});
...
// 정렬된 글 목록 로드 - 전역 함수
window.loadSortedPosts = function(page) {
    const container = document.getElementById('posts-container');
    container.innerHTML = '<div class="loading-message"><p>글을 불러오는 중...</p></div>';
    
    const url = window.currentSort === 'latest' 
        ? '<%=root%>/api/community/posts?page=' + page
        : '<%=root%>/api/community/posts?sort=popular&page=' + page;
...
window.deletePost = function(postId) {
    ...
    fetch('<%=root%>/api/community/post/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: params.toString()
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            // 글 상세 페이지에서 목록으로 돌아가기 (카테고리 이름 동적 설정)
            let categoryName = '헌팅썰';
            switch(window.currentCategoryId) {
                case 1: categoryName = '헌팅썰'; break;
                case 2: categoryName = '코스 추천'; break;
                case 3: categoryName = '같이 갈래'; break;
                default: categoryName = '커뮤니티'; break;
            }
            window.loadCategoryPosts(window.currentCategoryId, categoryName);
        } else {
