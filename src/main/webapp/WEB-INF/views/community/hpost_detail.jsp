...
<%
    ...
    // 카테고리 정보 설정
    int categoryId = post.getCategory_id();
...
<div class="post-detail-container" data-post-id="<%= post.getId() %>">
    ...
    <div class="post-content">
        ...
        <% if(post.getPhoto1() != null && !post.getPhoto1().isEmpty()) { %>
            <div class="post-photos">
                <img src="<%= root %>/api/community/images/<%= post.getPhoto1() %>" alt="첨부사진1" class="post-photo" loading="lazy" onclick="openImageModal('<%= root %>/api/community/images/<%= post.getPhoto1() %>', 1)">
                <% if(post.getPhoto2() != null && !post.getPhoto2().isEmpty()) { %>
                    <img src="<%= root %>/api/community/images/<%= post.getPhoto2() %>" alt="첨부사진2" class="post-photo" loading="lazy" onclick="openImageModal('<%= root %>/api/community/images/<%= post.getPhoto2() %>', 2)">
                <% } %>
                <% if(post.getPhoto3() != null && !post.getPhoto3().isEmpty()) { %>
                    <img src="<%= root %>/api/community/images/<%= post.getPhoto3() %>" alt="첨부사진3" class="post-photo" loading="lazy" onclick="openImageModal('<%= root %>/api/community/images/<%= post.getPhoto3() %>', 3)">
                <% } %>
            </div>
        <% } %>
    </div>
    ...
    <div class="post-navigation">
        <a href="javascript:void(0)" onclick="window.goBackToList()" class="back-btn">← 목록으로 돌아가기</a>
    </div>
</div>

<script>
// 현재 글의 카테고리 정보를 전역 변수에 저장
